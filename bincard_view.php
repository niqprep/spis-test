...
else
{
	...
	else
	{
		$qry = "SELECT * FROM itrmc_db01.stocks 
			...
			and stocks.s_status = 'Active'
			and (stocks.category = 'HK'
					or stocks.category = 'OS')
		...
		/*
					...
					SELECT * FROM itrmc_db01.stocks as s
					INNER JOIN itrmc_db01.bal_log as bl on s.id = bl.stockid
					left outer JOIN itrmc_db01.addbalpo as a on bl.log_id = a.ballog_id
					left outer join itrmc_db01.invoice as i on a.invoice_id = i.inv_id
					where s.id = '201500006'
					and s.s_status = 'Active'
					and (s.category = 'HK'
							or s.category = 'OS')
					and (bl.dte_credeb between '2015-01-18' and '2015-06-19')
					ORDER BY bl.createdon;
					*/

		//get po number and invoice number
		$qrypo = "SELECT * FROM itrmc_db01.stocks 
					inner JOIN itrmc_db01.bal_log on stocks.id = bal_log.stockid
		...
		<html>
			...
			if(isset($item))
			{	
				echo '<title>Bincard - '.$item.', '.$desc.' </title>';
			}	
