...
$qryn = "SELECT purchase_order.category as `cat`, dept.`name` as dept, pr_id, pr.pr_no, pr.section, section.sect_id, sectname, podate, po_number, supplier.`name` as supp, purchase_order.id
		...
		from purchase_order 
		 ...
		 where purchase_order.status = 'fromMCC'";
			...
			while($rown = $mysqln->fetch_assoc())	
				{
					...
					if ($cat == 'HK' OR $cat == 'OS') 
					{
						$qryi = "SELECT stocks.item_name, stocks.`desc` as idesc, stocks.unit, po_poitems.createddte, po_poitems.tobudgetdte, po_poitems.approveddte, po_poitems.rcvdapprovdte
							from purchase_order 
								left outer join po_poitems on purchase_order.id = po_poitems.po_id
								left outer join stocks on po_poitems.poitem_id = stocks.id
					...
					{
						$qryi = "SELECT othstocks.item_name, othstocks.`desc` as idesc, othstocks.unit, po_poitems.createddte, po_poitems.tobudgetdte, po_poitems.approveddte, po_poitems.rcvdapprovdte
							from purchase_order 
								left outer join po_poitems on purchase_order.id = po_poitems.po_id
					...
					<td><a href="poview.php?djm='.$poid.'" title="View" target="_new"> <img src="images\icons\view3.png" id="utils" ></a>
						...
						<a href="forwardto.php?djm='.$poid.'&<3=toSupplier&mpp='.$pono.'" title="Received by Supplier" target="_top"> <img src="images\icons\forward.png" id="utils" ></a>
