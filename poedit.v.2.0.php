...
if ($mysql->num_rows > 0)
{
	while($row = $mysql->fetch_assoc())
	{
		...
		//get the items list
		$qrypoitem = "SELECT po_poitems.id as po_poitemsid, stocks.id as stockid, 
		...
		{						
			...
			{
				$po_poitemid[$ctritem] = $rowb['po_poitemsid'];
				$stockid[$ctritem] = $rowb['stockid'];
				$poitem[$ctritem]= $rowb['item_name'];
				$poitemunit[$ctritem]= $rowb['unit'];
				$poitemdesc[$ctritem]= $rowb['desc'];
