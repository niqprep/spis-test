...
{	
	...
	$currdate = Date("Y-m-d h:i:s A");
		...
		//check first if there are duplicate invoice number
		$qryinv = "SELECT * FROM itrmc_db01.invoice where invoice = '$inv'";
		$mysqlinv = $conn->query($qryinv);
		if ($mysqlinv->num_rows > 0 )
		{
			//get id of the invoice 
			while ($rw = $mysqlinv-> fetch_assoc()) 
			{
				$invid = $rw['inv_id'];
				$amtx = $rw['amt'];
			}
			$amt = $amt + $amtx;
			//update invoice tbl if inv no. already exists
			$sql06 = "UPDATE itrmc_db01.invoice 
			...
			$sql03 = "INSERT INTO itrmc_db01.addbalpo (po_no, invoice_id, ballog_id, remarks, createdby, createdon)
					VALUES ('$pono', '$invid', '$id02', '$remarks', '$userid', '$currdate')";
			$mysql03 = $conn->query($sql03);

		}
		else
		{
			//add invoice into the invoice tbl
			$sql06 = "INSERT INTO itrmc_db01.invoice (invoice, invdte, status, createdby, createdon, amt)
					VALUES ('$inv', '$invdte', '', '$userid', '$currdate', '$amt')";
			$mysql06 = $conn->query($sql06);
			$invid1 = $conn->insert_id; //get the last inserted id

			//add a record into the addbalpo tbl
			$sql03 = "INSERT INTO itrmc_db01.addbalpo (po_no, invoice_id, ballog_id, remarks, createdby, createdon)
					VALUES ('$pono', '$invid1', '$id02', '$remarks', '$userid', '$currdate')";
			$mysql03 = $conn->query($sql03);
		}
		...
		else
		{
			...
			echo '<script type="text/javascript">'; 
			echo 'alert("Stock/s has been successfully added on the inventory.\\n Thank you.");'; 
			echo 'window.location.href = "issueoth.php?cat='.$cat.'";';
			echo '</script>';
		}
